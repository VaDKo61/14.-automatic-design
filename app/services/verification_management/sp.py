...
from .matches import get_secondary_items, get_exact_matches, get_fuzzy_matches, match_items, zip_last
...
def save_sp_assem(sw_assem, dir_sp: str, number_project: str) -> str | None:
    ...
    if not sp_assem:
        raise SwTableError
    sp_assem.BomFeature.RoutingComponentGrouping = 2
    # sp_assem.Sort(2, True)
...
def create_verification_excel(dir_sp: str, dir_sp_assem: str):
    ...
    wb_compare.save(str(Path(dir_sp).parent / Path(dir_sp).stem) + ' сравнение.xlsx')
